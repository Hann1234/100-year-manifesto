...
function Manifesto({ admin_page = false, user_name = "" }) {
  ...
  //This gets all information to fill on the manifesto
  useEffect(() => {
    if (!admin_page) {
  ...
  //Declarations of manifesto variables
  ...
  //Clicking manifesto will take you to a full view of it
  const handleClick = () => {
    dispatch({ type: "CLEAR_NEXT_BUTTON" });
    history.push("myManifesto");
  };

  //This if for separating the guiding principles in two arrays so it appears in to columns
  // when there more than 5
        ...
        >
          <Grid className="smallColumn">
            <div className="mission">
              MISSION: {mission.length !== 0 && mission[0].manifesto_text}
            </div>
            <div className="smallColumnAutoFill dottedBottom">
              ...
              {mantras.length !== 0 &&
                mantras.map((mantra) => {
                  ...
                  // This separate a mantra in two lines when its above 30 characters or 5 words or more
                  if (
                    mantra.manifesto_text.length >= 30 ||
                    stringArray.length > 4
                  ) {
                    for (
                      let index = 0;
                      index <= stringArray.length - 3;
                      index++
                    ) {
                  ...
                  // This shows on the manifesto the svg element
                  return textArray.map((element) => (
                    <ManifestoSvgElement manifestoText={element} />
                  ));
                })}
            </div>
            <div className="coreValues smallColumnAutoFill dottedBottom">
              ...
              {coreValues.length !== 0 ? (
                coreValues.map((value) => (
                  <ManifestoSvgElement
                    manifestoText={value.manifesto_text}
                    key={value.id}
                  />
                ))
              ) : (
              ...
              <Grid className="lifeGoals bigColumnAutoFill">
                {lifeGoals.length !== 0 &&
                  lifeGoals.map((textItem) => (
                    <Grid className="lifeGoalItem" key={textItem.id}>
                      {textItem.manifesto_text}{" "}
                    </Grid>
                  ))}
              </Grid>
            </div>
            <div>
              <Grid item className="principlesTitle blueBar">
                Guiding Principles
              </Grid>
              <Grid
                container
                className="principles bigColumnAutoFill"
                spacing={1}
              >
                {/* This shows to columns instead of one when guiding principles  */}
                {guidingPrinciples.length !== 0 &&
                guidingPrinciples.length < 6 ? (
                  guidingPrinciples.map((textItem) => {
                    return (
                      <Grid
                        item
                        xs={12}
                        className={gpFontSize(textItem.manifesto_text)}
                        key={textItem.id}
                      >
                        {textItem.manifesto_text}
                        <br /> {textItem.source}
...
//changes fontsize of guiding principles according with it's total characters
