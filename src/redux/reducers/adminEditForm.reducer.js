    ...
    case "SET_EDIT_ON_DATE":
      ...
      if (action.payload.edit_data) {
        if (
          ...
          state.findIndex(
            (row) =>
              row.html_id === action.payload.edit_data.html_id &&
              row.page_id === action.payload.edit_data.page_id
          ) >= 0
        ) {
          ...
          return [...state, action.payload.edit_data].filter(
            (row) => row.page_id === action.payload.edit_data.page_id
          );
        }
      } else {
        // if database response was empty, remove that value from reducer
        return [...state].filter((row) => {
          return (
            row.html_id !== action.payload.html_id ||
            row.page_id !== action.payload.page_id
          );
        });
      }
    default:
