...
function* fetchPageEdits(action) {
  try {
    const pageEdits = yield axios.get(
      `/api/adminEditForm/${action.payload.page_id}`
    );
...
function* fetchEditOnDate(action) {
  try {
    const editOnDate = yield axios.get(
      `/api/adminEditForm/edit_on_date/${
        action.payload.page_id
      }/?edit_date=${encodeURIComponent(
        action.payload.edit_date
      )}&html_id=${encodeURIComponent(action.payload.html_id)}`
    );
    yield put({
      type: "SET_EDIT_ON_DATE",
      payload: {
        edit_data: editOnDate.data[0],
        page_id: action.payload.page_id,
        html_id: action.payload.html_id,
      },
    }); // Loads page edits on date into reducer
  } catch (error) {
...
function* addPageEdits(action) {
  try {
    ...
    yield put({
      type: "FETCH_PAGE_EDITS",
      payload: { page_id: action.payload.page_id },
    }); // Loads the updated page edits into reducer
  } catch (error) {
