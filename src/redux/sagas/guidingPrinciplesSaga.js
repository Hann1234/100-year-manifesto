...
function* fetchGuidingPrinciples() {
  try {
    ...
    yield put({
      type: "SET_GUIDING_PRINCIPLES",
      payload: guidingPrinciples.data,
    }); //Loads guiding principles into reducer
  } catch (error) {
...
function* fetchSpecificUserGuidingPrinciples(action) {
  try {
    const guidingPrinciples = yield axios.get(
      `/api/guidingPrinciples/${action.payload.id}`
    );
    yield put({
      type: "SET_GUIDING_PRINCIPLES",
      payload: guidingPrinciples.data,
    }); //Loads specific user's guiding principles into reducer
  } catch (error) {
...
function* guidingPrinciplesSaga() {
  ...
  yield takeLatest(
    "FETCH_USER_GUIDING_PRINCIPLES",
    fetchSpecificUserGuidingPrinciples
  );
