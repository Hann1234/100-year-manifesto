...
/**
 * GET route for specific user's guiding_principles
 */
router.get("/:id", rejectUnauthenticated, (req, res) => {
  if (req.user.role === "admin" || req.user.role === "superadmin") {
    ...
    pool
      ...
      .catch((error) => {
        console.log("Error GETting specific users guiding_principles", error);
        res.sendStatus(500);
      });
  } else {
    console.log(
      "Permission denied GETting specific users guiding_principles",
      error
    );
    res.sendStatus(403);
  }
...
router.post("/", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(qText, [uId, source, manifestoText])
    .then(() => {
      res.sendStatus(201);
    })
    .catch((error) => {
...
router.put("/:id", rejectUnauthenticated, (req, res) => {
  ...
  pool
    .query(qText, [source, manifestoText, id, uId])
    .then(() => {
      res.sendStatus(201);
    })
    .catch((error) => {
